...
export default function FollowButton({ currentUserData, targetUser }: FollowButtonProps) {
  ...
  const isFollowing = currentUserData?.following?.includes(targetUser.uid) ?? false;
  const hasSentRequest = targetUser.followRequests?.some((req: any) => req.uid === currentUserData?.uid) ?? false;
