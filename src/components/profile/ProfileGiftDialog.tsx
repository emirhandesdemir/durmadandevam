// src/components/profile/ProfileGiftDialog.tsx
'use client';
...
interface ProfileGiftDialogProps {
  isOpen: boolean;
  onOpenChange: (open: boolean) => void;
  recipient: UserProfile;
}

export default function ProfileGiftDialog({ isOpen, onOpenChange, recipient }: ProfileGiftDialogProps) {
  ...
  const handleSendGift = async () => {
    ...
    try {
        ...
        toast({ description: `${recipient.username} adlı kullanıcıya hediye gönderildi!` });
        onOpenChange(false);
    ...
    <Sheet open={isOpen} onOpenChange={onOpenChange}>
      <SheetContent side="bottom" className="h-[60dvh] flex flex-col p-0">
        <SheetHeader className="p-4 border-b">
          <SheetTitle>Hediye Gönder: {recipient.username}</SheetTitle>
          <SheetDescription>
