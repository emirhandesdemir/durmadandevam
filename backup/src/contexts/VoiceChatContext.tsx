...
export function VoiceChatProvider({ children }: { children: ReactNode }) {
    ...
    useEffect(() => {
        ...
        console.log("Managing peer connections. Current peers:", currentPeerIds, "New participants:", newParticipantIds);

        newParticipantIds.forEach(uid => {
            if (!currentPeerIds.includes(uid)) {
                console.log("Creating new peer connection to:", uid);
    ...
    useEffect(() => {
        const handleBeforeUnload = (e: BeforeUnloadEvent) => {
             if (isConnected && activeRoom && user) {
                // This is a synchronous operation but we call an async function.
                // It's not guaranteed to complete, but it's the best we can do.
